...
def _frontend_dist_dir() -> str:
    """解析前端构建产物目录，兼容从 backend 目录直接启动。"""
    return os.path.abspath(os.path.join(os.path.dirname(__file__), settings.FRONTEND_DIST_DIR))
...
if _frontend_enabled():
    ...
    async def frontend_spa_fallback(full_path: str):
        blocked_prefixes = ("api", "docs", "redoc", "openapi.json", "health", "storage")
        if full_path.startswith(blocked_prefixes):
            raise HTTPException(status_code=404, detail="Not Found")

        candidate_path = os.path.join(frontend_dist_dir, full_path)
